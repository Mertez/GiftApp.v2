...
export const PersonGifCat = ({ catItems, navigation }) => {
    ...
    const renderItem = ({ item, index }) => (
        //console.log(item)
        <Spacer key={item.original + "L" + "_" + index} position="left" size="sm">
            <Spacer key={item.original + "R" + "_" + index} position="rightx" size="sm">
                <Spacer key={item.original + "T" + "_" + index} position="top" size="sm">
                    <Spacer key={item.original + "B" + "_" + index} position="bottomx" size="sm">
                        <TouchableOpacity key={item.original + "TO" + "_" + index}
                        ...
                        >
                            <Row key={item.original + "ROW" + "_" + index}>
                                {/* <Image source={require('../../../../assets/ams24.png')} style={{ height: 24 }} /> */}
                                <Text style={{ color: 'black' }} key={item.original + "TEXT" + "_" + index}>{item.original}</Text>
                            </Row>
